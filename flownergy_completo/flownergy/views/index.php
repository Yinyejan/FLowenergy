...
$imagenes = [
    'esp32' => 'https://upload.wikimedia.org/wikipedia/commons/thumb/2/20/ESP32_Espressif_ESP-WROOM-32_Dev_Board.jpg/960px-ESP32_Espressif_ESP-WROOM-32_Dev_Board.jpg',
    'pzem'  => 'https://how2electronics.com/wp-content/uploads/2025/02/PZEM-004T-V3.0-368x324.jpg',
    'panel' => 'https://images.unsplash.com/photo-1753272691001-4d68806ac590?auto=format&fit=crop&fm=jpg&q=80&w=1600',
];
...
.image-card{position:relative;min-height:310px;border:1px solid var(--line);border-radius:8px;overflow:hidden;background:#081016}
.image-card img{width:100%;height:100%;min-height:310px;display:block;object-fit:cover;filter:saturate(1.06) contrast(1.04)}
.image-card::after{content:"";position:absolute;inset:0;background:linear-gradient(to top,rgba(3,7,8,.78),transparent 62%)}
.image-caption{position:absolute;left:18px;right:18px;bottom:18px;z-index:2}
.image-caption strong{display:block;color:var(--text);font-size:1.25rem;margin-top:8px}
.image-caption span{display:block;color:var(--muted);line-height:1.55;margin-top:6px}
.inline-photo{width:100%;height:220px;object-fit:cover;border-radius:8px;border:1px solid var(--line);margin-bottom:18px;background:#081016}
.mission-heading{font-size:clamp(1.65rem,2.6vw,2.45rem);line-height:1.04;letter-spacing:-.035em;margin-top:12px}
.wide-band{
...
.chart-box{position:relative;width:100%;height:320px;min-height:320px}
.chart-box canvas{display:block;width:100%!important;height:100%!important}
.live-chart-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;margin-bottom:18px}
.live-chart-meta{display:flex;gap:12px;flex-wrap:wrap;color:var(--muted);font-size:.82rem}
...
<section class="section fade-in-scroll">
  ...
  <div class="visual-grid" style="margin-bottom:18px;">
    <div class="image-card">
      <img src="<?= htmlspecialchars($imagenes['esp32']) ?>" alt="Placa ESP32 WROOM-32">
      <div class="image-caption">
        <span class="kicker">Microcontrolador</span>
        <strong>ESP32 WROOM-32</strong>
        <span>Lee, procesa y transmite la telemetria electrica por Bluetooth.</span>
      </div>
    </div>
    <div class="image-card">
      <img src="<?= htmlspecialchars($imagenes['pzem']) ?>" alt="Sensor PZEM-004T V3.0">
      <div class="image-caption">
        <span class="kicker">Sensor electrico</span>
        <strong>PZEM-004T V3.0</strong>
        <span>Mide voltaje, corriente, potencia, energia, frecuencia y factor de potencia.</span>
      </div>
    </div>
  </div>
  <div class="grid-3">
...
<section class="section fade-in-scroll">
  <div class="grid-2">
    <div class="image-card">
      <img src="<?= htmlspecialchars($imagenes['esp32']) ?>" alt="ESP32 usado como controlador IoT">
      <div class="image-caption">
        <span class="kicker">Control IoT</span>
        <strong>ESP32 como cerebro del prototipo.</strong>
        <span>Recibe los datos del sensor y los envia al navegador para la medicion en vivo.</span>
      </div>
    </div>
    <div class="image-card">
      <img src="<?= htmlspecialchars($imagenes['pzem']) ?>" alt="Modulo PZEM-004T para medicion electrica">
      <div class="image-caption">
        <span class="kicker">Medicion electrica</span>
        <strong>PZEM-004T como sensor principal.</strong>
        <span>Convierte variables electricas reales en datos listos para graficar y guardar.</span>
      </div>
    </div>
  </div>
</section>
...
<section class="section fade-in-scroll">
  ...
  <p class="sec-sub">lema, solucion, impacto, ODS y demostracion.</p>
  <div class="speech-panel">
    <div>
      ...
      <p style="color:var(--muted);line-height:1.75;text-align:center;margin-top:16px;">https://screenapp.io/app/v/pGyQIrbwcO </p>
    </div>
    <a href="<?= htmlspecialchars("https://screenapp.io/app/v/pGyQIrbwcO") ?>" target="_blank" class="btn btn-gold btn-full" <?= "https://screenapp.io/app/v/pGyQIrbwcO" === '#' ? 'onclick="event.preventDefault(); alert(\'Edita $video_speech_url en index.php con el link de tu speech.\');"' : '' ?>>Ver video speech</a>
  </div>
</section>
...
<section class="section fade-in-scroll">
  ...
  <div class="grid-2" style="margin-bottom:18px;">
    <div class="card">
      <img class="inline-photo" src="<?= htmlspecialchars($imagenes['panel']) ?>" alt="Panel electrico industrial monitoreado">
      <span class="kicker">Identidad</span>
      <h3 class="mission-heading">Mision</h3>
      <p style="margin-top:18px;color:var(--muted);line-height:1.8;font-size:1.04rem;"><?= $proyecto['mision'] ?></p>
    </div>
    <div class="card">
      <img class="inline-photo" src="<?= htmlspecialchars($imagenes['esp32']) ?>" alt="ESP32 como base tecnologica de Flownergy">
      <span class="kicker">Futuro</span>
      <h3 class="mission-heading">Vision</h3>
...
<section class="section fade-in-scroll" style="text-align:center;">
  ...
  <div class="image-card" style="max-width:760px;margin:0 auto 28px;text-align:left;">
    <img src="<?= htmlspecialchars($imagenes['panel']) ?>" alt="Instalacion electrica industrial para monitoreo">
    <div class="image-caption">
      <span class="kicker">Instalacion piloto</span>
      <strong>Datos reales para decisiones reales.</strong>
      <span>Flownergy puede adaptarse a una maquina, tablero o punto de consumo electrico.</span>
    </div>
  </div>
...
<section class="login-shell fade-in-scroll">
  <div class="login-pitch">
    ...
    <img class="inline-photo" src="<?= htmlspecialchars($imagenes['esp32']) ?>" alt="ESP32 para conexion Bluetooth del sistema">
...
<?php else: ?>
  ...
  <div class="card" style="margin-bottom:18px;">
    <div class="live-chart-head">
      <div>
        <span class="kicker">Grafico en vivo</span>
        <h3 style="font-size:1.35rem;margin-top:12px;">Comportamiento electrico</h3>
        <p style="color:var(--muted);margin-top:5px;">Se actualiza con cada lectura recibida por Bluetooth.</p>
      </div>
      <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
        <div class="live-chart-meta">
          <span>Potencia W</span><span>Voltaje V</span><span>FP x100</span>
        </div>
        <button type="button" class="btn btn-blue btn-sm" onclick="exportLiveCsv()">Exportar CSV</button>
        <button type="button" class="btn btn-outline btn-sm" onclick="exportLiveChartImage()">Exportar grafico</button>
      </div>
    </div>
    <div class="chart-box">
      <canvas id="grafica-vivo"></canvas>
    </div>
  </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initBackTop();
  initMeasurementNavigationGuard();
});
...
function initMeasurementNavigationGuard() {
  document.querySelectorAll('a[href^="?ver="]').forEach((link) => {
    link.addEventListener('click', (event) => {
      const targetUrl = new URL(link.getAttribute('href'), window.location.href);
      const currentSection = new URL(window.location.href).searchParams.get('ver') || 'inicio';
      const nextSection = targetUrl.searchParams.get('ver') || 'inicio';
      if (!btConnected || currentSection !== 'medicion' || nextSection === 'medicion') return;
      event.preventDefault();
      window.open(targetUrl.href, '_blank', 'noopener');
      showSystemAlert('Dashboard abierto en otra pestana para mantener activa la conexion Bluetooth de esta medicion.', 'alerta-blue');
    });
  });
}
...
function renderLiveChart() {
  const canvas = document.getElementById('grafica-vivo');
  if (!canvas) return;
  ...
  liveChart = new Chart(ctx, {
    ...
    data: {
      labels: [],
      datasets: [
        { label: 'Potencia (W)', data: [], borderColor: '#f7c948', backgroundColor: 'rgba(247,201,72,.12)', borderWidth: 2, tension: .35, pointRadius: 0, fill: true },
        { label: 'Voltaje (V)', data: [], borderColor: '#34d6ff', borderWidth: 2, tension: .35, pointRadius: 0 },
        { label: 'FP x100', data: [], borderColor: '#31e981', borderWidth: 2, tension: .35, pointRadius: 0 }
      ]
    },
    options: {
      responsive: true,
...
function updateLiveChart(data) {
  ...
  const reading = {
    fecha_iso: timestamp.toISOString(),
  ...
  const chartReadings = liveReadings.slice(-LIVE_CHART_LIMIT);
  if (!liveChart) {
    drawLiveCanvas();
    return;
  }
  liveChart.data.labels = chartReadings.map((item) => item.hora);
  liveChart.data.datasets[0].data = chartReadings.map((item) => item.potencia);
  liveChart.data.datasets[1].data = chartReadings.map((item) => item.voltaje);
  liveChart.data.datasets[2].data = chartReadings.map((item) => item.fp * 100);
  liveChart.update('none');
}

function drawLiveCanvas() {
  if (typeof Chart !== 'undefined' && liveChart) return;
  const canvas = document.getElementById('grafica-vivo');
  if (!canvas) return;
  const box = canvas.parentElement;
  const width = box ? box.clientWidth : 760;
  const height = box ? box.clientHeight : 320;
  ...
  const pad = { left: 46, right: 18, top: 26, bottom: 34 };
  const plotW = width - pad.left - pad.right;
  const plotH = height - pad.top - pad.bottom;
  const gradient = ctx.createLinearGradient(0, 0, 0, height);
  gradient.addColorStop(0, 'rgba(52,214,255,.08)');
  gradient.addColorStop(1, 'rgba(247,201,72,.03)');
  ...
  if (!liveReadings.length) {
    ctx.fillStyle = '#9aa8b6';
    ctx.font = '600 14px Inter, sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText('Esperando lecturas Bluetooth...', width / 2, height / 2);
    return;
  }

  const chartReadings = liveReadings.slice(-LIVE_CHART_LIMIT);
  const series = [
    { label: 'Potencia', color: '#f7c948', values: chartReadings.map((item) => item.potencia) },
    { label: 'Voltaje', color: '#34d6ff', values: chartReadings.map((item) => item.voltaje) },
    { label: 'FP x100', color: '#31e981', values: chartReadings.map((item) => item.fp * 100) }
  ];
  ...
  const xFor = (index) => pad.left + (plotW * index) / Math.max(1, chartReadings.length - 1);
  const yFor = (value) => pad.top + plotH - ((value - min) / (max - min)) * plotH;

  ctx.fillStyle = '#667584';
  ctx.font = '12px Inter, sans-serif';
  ctx.textAlign = 'right';
  ctx.fillText(max.toFixed(0), pad.left - 10, pad.top + 4);
  ctx.fillText(min.toFixed(0), pad.left - 10, pad.top + plotH);
...
function exportLiveCsv() {
  if (!liveReadings.length) {
    showSystemAlert('Aun no hay lecturas en vivo para exportar.', 'alerta-yellow');
    return;
  }
  const headers = [
    'Fecha ISO',
  ...
  const rows = liveReadings.map((item) => [
    item.fecha_iso,
    item.hora,
    item.voltaje,
    item.corriente,
  ...
  link.href = url;
  link.download = `medicion_en_vivo_${stamp}.csv`;
...
function exportLiveChartImage() {
  const source = document.getElementById('grafica-vivo');
  if (!source) {
    showSystemAlert('No se encontro el grafico para exportar.', 'alerta-yellow');
    return;
  }
  const width = source.width || source.clientWidth;
  const height = source.height || source.clientHeight;
  if (!width || !height) {
    showSystemAlert('El grafico aun no esta listo para exportar.', 'alerta-yellow');
    return;
  }
  ...
  link.download = `grafico_medicion_${stamp}.png`;
  document.body.appendChild(link);
  link.click();
  link.remove();
}
